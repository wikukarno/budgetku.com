<template>
  <MainLayout title="Payment Methods - Customer" page-title="Payment Methods" :requireUnlock="true">
...
async function btnEditPaymentMethod(uuid){
  ...
  modal.show();
  try {
    const res = await window.axios.get('/pages/customer/payment-method/show', { params: { uuid } });
...
async function btnDeletePaymentMethod(uuid){
  window.Swal.fire({
    ...
    (async () => {
      try {
        await window.axios.delete('/pages/customer/payment-method/delete', { data: { uuid } });
...
async function onSubmit(){
  ...
  try {
    ...
    await window.axios.post('/pages/customer/payment-method/store', fd);
...
onMounted(() => {
      ...
      { data: null, searchable: false, orderable: false, render: function(data, type, row){
          ...
          return `
            <div class="d-flex justify-content-end flex-column flex-sm-row flex-wrap gap-2">
              <button type="button" class="btn btn-warning btn-sm text-white" data-action="edit-pm" data-uuid="${uuid}">
                <i class="ri-edit-line"></i><span class="ms-1">Edit</span>
              </button>
              <button type="button" class="btn btn-danger btn-sm text-white" data-action="delete-pm" data-uuid="${uuid}">
  ...
  document.addEventListener('click', function(e){
    ...
    if (!uuid) return;
    if (act === 'edit-pm') { e.preventDefault(); btnEditPaymentMethod(uuid); }
    if (act === 'delete-pm') { e.preventDefault(); btnDeletePaymentMethod(uuid); }
  });
});
