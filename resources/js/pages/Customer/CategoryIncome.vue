<template>
  <MainLayout title="Income Category" page-title="Income Category" :requireUnlock="true">
        ...
        <div class="modal-content">
          ...
          <form id="form-tambah-kategori-income" @submit.prevent="onSubmit">
            <div class="modal-body">
              <input type="hidden" name="uuid" id="id_category_income" v-model="form.uuid" />
...
async function updateKategoriIncome(uuid) {
  form.value = { uuid: '', name: '' };
  ...
  try {
    showPreloader();
    const res = await window.axios.get('/pages/customer/category/income/show', { params: { uuid } });
    const data = res.data || {};
...
function deleteKategoriIncome(uuid) {
  window.Swal.fire({
    ...
    (async () => {
      try {
        showPreloader();
        const res = await window.axios.delete('/pages/customer/category/income/delete', { data: { uuid } });
...
async function onSubmit() {
  ...
  try {
    ...
    let armored = null;
    try {
      armored = await encryptForSelf(form.value.name);
    } catch (e) { armored = null; }
    ...
    await window.axios.post('/pages/customer/category/income/store', fd);
...
onMounted(async () => {
  ...
  try {
    ...
    console.warn('[CategoryIncome] Failed to initialize E2EE:', error);
  }
  ...
  const dt = $('#categoryIncomeTable').DataTable({
    processing: true,
    serverSide: false,
      ...
      { data: null, searchable: false, orderable: false, render: function(data, type, row){
          ...
          return `
            <div class="d-flex justify-content-end flex-column flex-sm-row flex-wrap gap-2">
              <button type="button" class="btn btn-warning btn-sm text-white" data-action="edit-ci" data-uuid="${uuid}">
                <i class="ri-edit-line"></i><span class="ms-1">Edit</span>
              </button>
              <button type="button" class="btn btn-danger btn-sm text-white" data-action="delete-ci" data-uuid="${uuid}">
  ...
  // Decrypt all rows after load so client search uses plaintext
  ...
  const onActionClick = (e) => {
    ...
    if (!uuid) return;
    if (act === 'edit-ci') { e.preventDefault(); updateKategoriIncome(uuid); }
    if (act === 'delete-ci') { e.preventDefault(); deleteKategoriIncome(uuid); }
  };
  document.addEventListener('click', onActionClick);
  // Optional: cleanup handler on page unload
...
async function decryptVisibleRows() {
  ...
  // Decrypt per row and replace cell text
