<template>
  <MainLayout title="Expense Category" page-title="Expense Category">
...
async function updateKategoriFinance(uuid) {
  ...
  try {
    showPreloader();
    const res = await window.axios.get('/pages/customer/category/expense/show', { params: { uuid } });
    const data = res.data || {};
...
function deleteKategoriFinance(uuid) {
  window.Swal.fire({
    ...
    (async () => {
      try {
        showPreloader();
        const res = await window.axios.delete('/pages/customer/category/expense/delete', { data: { uuid } });
        window.$('#categoryExpenseTable').DataTable().ajax.reload();
        window.showCustomAlert?.('success', res?.data?.message || 'Deleted');
      } catch (e) {
...
async function onSubmit() {
  ...
  try {
    ...
    // Try encrypt using user's public key
    let armored = null;
    try { armored = await encryptForSelf(form.value.name); } catch (e) { armored = null; }
    if (armored) {
    ...
    await window.axios.post('/pages/customer/category/expense/store', fd);
    const modal = window.bootstrap.Modal.getInstance(document.getElementById('categoryExpenseModal'));
    modal?.hide();
    window.$('#categoryExpenseTable').DataTable().ajax.reload();
...
onMounted(async () => {
  ...
  try {
    ...
    console.warn('[CategoryExpense] Failed to initialize E2EE:', error);
  }
  ...
  const dt = $('#categoryExpenseTable').DataTable({
    processing: true,
    serverSide: false,
    ...
    columns: [
      ...
      {
        data: 'name_category_finances',
        name: 'name_category_finances',
        render: function (data, type, row) {
          if (type === 'filter' || type === 'sort') {
            return row && row.name_category_finances ? row.name_category_finances : '';
          }
          try {
      ...
      { data: null, searchable: false, orderable: false, render: function(data, type, row){
          ...
          return `
            <div class="d-flex justify-content-end flex-column flex-sm-row flex-wrap gap-2">
              <button type="button" class="btn btn-warning btn-sm text-white" data-action="edit-ce" data-uuid="${uuid}">
                <i class="ri-edit-line"></i><span class="ms-1">Edit</span>
              </button>
              <button type="button" class="btn btn-danger btn-sm text-white" data-action="delete-ce" data-uuid="${uuid}">
    ...
    initComplete: function () {
      ...
      // Rapikan header agar sejajar; styling tinggi dihandle global CSS DataTables
  ...
  const onActionClick = (e) => {
    ...
    if (!uuid) return;
    if (act === 'edit-ce') { e.preventDefault(); updateKategoriFinance(uuid); }
    if (act === 'delete-ce') { e.preventDefault(); deleteKategoriFinance(uuid); }
  };
  document.addEventListener('click', onActionClick);
  window.addEventListener('beforeunload', () => document.removeEventListener('click', onActionClick));
});

// ---- E2EE helpers (same pattern as Income) ----
...
async function decryptVisibleRows() {
  const $ = window.$;
  const api = $('#categoryExpenseTable').DataTable();
  // Try decrypt even if store not yet unlocked; fallback will use sessionStorage
...
async function decryptAllRows() {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (d && d.name_category_finances === '[encrypted]' && d.name_category_finances_pgp) {
      try {
        const txt = await decryptArmor(d.name_category_finances_pgp);
        d.name_category_finances = txt;
        api.row(idx).data(d);
      } catch {}
    }
  }
