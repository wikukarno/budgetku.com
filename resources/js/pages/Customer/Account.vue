          ...
          <div class="card-body py-38 px-4">
            <div class="mb-5">
              <h3 class="text-white fw-semibold">
                Welcome Back, <span class="text-danger-div">{{ user?.name }}!</span>
              </h3>
              ...
              <div class="card-body p-4">
                ...
                <div class="tab-content" id="settingsTabContent">
                  ...
                  <div class="tab-pane fade show active" id="account" role="tabpanel">
                    ...
                    <form id="account-form" :action="`/pages/customer/account/update/${user?.uuid}`" method="POST" @submit.prevent="submitProfile">
                  ...
                  <div class="tab-pane fade" id="password" role="tabpanel">
                    ...
                    <form id="change-password-form" action="/pages/customer/account/password/update" method="POST" @submit.prevent="submitPassword">
                      <input type="hidden" name="_method" value="PUT" />
                      <div class="mb-3">
                  ...
                  <div class="tab-pane fade" id="twofa" role="tabpanel">
                    ...
                    <button class="btn btn-primary" id="btn-enable-2fa" v-show="!twoFactorEnabled" @click="showModalTwoFA">
                      <i class="bi bi-shield-lock"></i> Enable Two-Factor Authentication
                    </button>

                    <div id="disable-2fa-section" v-show="twoFactorEnabled">
...
function showModalTwoFA() {
  ...
  // Trigger backend setup and UI updates handled by page JS (account.js) or implement inline as needed
  ...
  window.axios.post('/2fa/setup', fd).then(res => {
    ...
    window.showCustomAlert?.('danger', 'Failed to enable 2FA');
  });
}
...
function disable2FA() {
  ...
  window.Swal?.fire({
    ...
    confirmButtonColor: '#d33', cancelButtonColor: '#6c757d', confirmButtonText: 'Yes, disable it'
  }).then(r => {
...
function deleteAccount() {
  window.Swal?.fire({
    ...
    icon: 'warning', showCancelButton: true,
    confirmButtonColor: '#d33', cancelButtonColor: '#6c757d', confirmButtonText: 'Yes, delete it!'
  }).then((result) => {
    if (!result.isConfirmed) return;
    window.axios.post('/pages/customer/account/delete', null, { params: { _method: 'DELETE' } })
      .then(res => {
        if (res.data?.status) {
          window.Swal.fire({ icon:'success', title:'Deleted!', text: res.data.message, timer: 2000, showConfirmButton:false })
...
onMounted(() => {
  // Activate tab based on URL hash and keep it updated
  try {
  ...
  // Bind E2EE buttons explicitly for SPA
  try {
